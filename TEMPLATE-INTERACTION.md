...
- **Agent CLI self-update mid-session breaks packet contracts.** cardgame-dad-relay #22 caught a peer-tool updater that ran `npm install -g @openai/codex@latest` from inside a Codex-hosted shell; the update killed the live session. The protocol-level concern is more subtle: even if the host survives the update, a session that spans the upgrade boundary has turns authored by different CLI versions with potentially different packet shapes. Future bisect/audit needs to know which CLI version authored each turn. Recommend adding an optional `authored_by: {agent, cli_version}` block at the turn packet level (sibling to existing peer metadata) and a session-level invariant that `cli_version` is stable across turns within the same session unless an explicit `session_cli_upgrade` marker is recorded. Validators should warn — not block — on silent version drift, and block on upgrades that cross a documented schema-breaking boundary.
- **Model-upgrade Bash/Read token inflation is session-level, not product-level.** Unity card-game #282 observed Opus 4.7 consuming ~1.5× the tokens 4.6 did on identical tool outputs and committed a shared `.claude/settings.json` with `BASH_MAX_OUTPUT_LENGTH` + `CLAUDE_CODE_FILE_READ_MAX_OUTPUT_TOKENS` caps. Because this is a session-runtime fix (not product- or schema-level), it belongs in every Claude Code-hosted DAD variant, not just downstream products. Shipping the shared caps in `en/.claude/settings.json` and `ko/.claude/settings.json` lets any new downstream inherit the default instead of re-deriving it after the first 4.7-hosted session. Per-developer overrides still go in `.claude/settings.local.json` (git-ignored). Revisit the numeric caps whenever the Claude model family moves — caps tuned for 4.7 will be wrong for a 4.8 that shifts token density again.

The open-source template's `en/tools/Validate-DadPacket.ps1` + `ko/tools/Validate-DadPacket.ps1` should eventually carry the schema assertions for the four bullets above (peer-symmetry check, status-with-reason pair check, repo_identity presence, cli_version continuity) so downstream relays inherit enforcement automatically rather than re-deriving it. Until then, the bullets are advisory and surfaced via `Validate-Upstream-Contract.ps1` in each downstream relay.
