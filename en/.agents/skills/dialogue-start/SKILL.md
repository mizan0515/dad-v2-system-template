---
name: dialogue-start
description: "Explicit-invocation skill to start a DAD v2 symmetric-turn dialogue session with Claude Code. Use when directly invoked via `$dialogue-start`. Use when an external critical review is valuable for medium/large tasks. Do not use for small tasks that a single agent can handle. Triggers: \"start dialogue session\", \"dialogue start\", \"collaborate with Claude Code\"."
---
...
- Example: `Start a DAD v2 session via $dialogue-start.`

## Preconditions

- The agent running this skill acts as **Codex**.
- The contract file is `AGENTS.md` (Claude Code's is `CLAUDE.md`).
- When producing the peer (Claude Code) prompt, start with `Read PROJECT-RULES.md first. Then read CLAUDE.md and DIALOGUE-PROTOCOL.md.`

## Procedure

1. Read `PROJECT-RULES.md` first, then read `AGENTS.md` and `DIALOGUE-PROTOCOL.md` to internalize the v2 protocol.
