...
8. Extend `ClassifyCommandCategory` to recognise `Get-Content`/`cat`/`type <path>` inside unwrapped PowerShell payloads so DAD-asset reads are not indistinguishable from arbitrary shell
9. Codex Windows compatibility matrix
