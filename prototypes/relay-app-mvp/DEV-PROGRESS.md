...
### Next priority
- Codex Windows compatibility matrix (next per AUTONOMOUS-DEV-PROMPT-COPY.txt Phase C sequence). Capture which Codex shell/file/git/MCP mechanisms behave differently on Windows vs. the documented defaults, so we can feed the findings back into classifier and sandbox work.
- Follow-up: add a dedicated DAD-asset band to `RelayApprovalPolicy` and surface `Get-Content`/`cat`/`type <path>` in the classifier so DAD reads are distinguishable from arbitrary shell (both surfaced by the DAD asset classification audit).
