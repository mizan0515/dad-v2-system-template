...
### Next priority
- Classifier follow-ups surfaced by the Windows matrix and DAD asset audit: (a) extend `RelayApprovalPolicy.ClassifyCommandCategory` to recognise `Get-Content`/`cat`/`type <path>` inside unwrapped PowerShell so reads are not indistinguishable from arbitrary shell; (b) add a dedicated DAD-asset band so writes under `Document/dialogue/**` are surfaced distinctly from generic file changes.
