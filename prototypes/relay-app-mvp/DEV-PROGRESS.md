...
### Next priority
- DAD asset classification audit (next per AUTONOMOUS-DEV-PROMPT-COPY.txt Phase C sequence). Capture how the broker classifies and governs DAD-template artifact operations (Document/dialogue/*, sessions, state.json writes) during a live relay session and whether current policy appropriately bands them.
