...
function Validate-PacketFile {
    ...
    if ($effectiveCloseoutKind -eq "peer_handoff") {
        ...
        if ((Test-AnyRegex -Text $handoffText -Patterns $metaOnlyPatterns) -and -not (Test-AnyRegex -Text $handoffText -Patterns $exceptionPatterns)) {
            Add-Warning -List $warnings -Message "handoff.closeout_kind=peer_handoff가 meta-only처럼 보입니다. recovery이거나 risk-gated verify relay가 아닌 이상 wording/state-summary sync/closure 작업은 현재 execution turn 안에서 끝내는 편이 맞습니다."
        }
    }
    elseif ($effectiveCloseoutKind -eq "final_no_handoff") {
        ...
        if (-not [string]::IsNullOrWhiteSpace($nextTask)) {
            Add-Issue -List $issues -Message "handoff.closeout_kind=final_no_handoff는 handoff.next_task를 비워 두어야 한다. 닫히는 세션에는 continuation이 없다. 남은 follow-up 작업이 있으면 세션을 봉인하기 전에 backlog로 admit한다."
        }
    }
        ...
        if ([string]::IsNullOrWhiteSpace($promptArtifact)) {
            ...
            if (-not (Test-Path $resolvedPromptArtifact)) {
                ...
                $requiredPromptPatterns = @(
                    ...
                    '^---\s*$',
                    '^허점이나 개선점이 있으면 직접 수정하고 diff를 보고하라\.\s*$',
                    '^수정할 것이 없으면 "변경 불필요, PASS"라고 명시하라\.\s*$',
                    '^중요: 관대하게 평가하지 마라\. "좋아 보인다" 금지\. 구체적 근거와 예시를 들어라\.\s*$'
                )
