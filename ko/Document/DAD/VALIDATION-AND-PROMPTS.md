...
downstream 저장소는 raw log보다 compact evidence/status artifact를 우선하는 편이 좋다.
다만 이 템플릿이 표준화해야 하는 compact artifact는 DAD 런타임 진실을
표현하는 경우로 한정한다.

DAD 계층에서 좋은 compact artifact 예:

- packet validator 결과 요약
- backlog admission 결과 요약
- prompt artifact 존재/부재 체크
- session closeout 정합성 marker

반대로 제품 dashboard, 제품 blocker label, 제품 remediation wording은 여러
제품에 걸쳐 truly generic해지기 전까지 이 DAD 템플릿으로 올리지 않는다.
`Validate-DadPacket.ps1`나 `Validate-DadBacklog.ps1`에 바깥 루프 compact-status
체크를 억지로 넣지 않는다. 그런 검증은 `autopilot-template` 또는 downstream
제품 저장소가 소유하는 별도 validator/doctor 경로에 두는 편이 맞다.
